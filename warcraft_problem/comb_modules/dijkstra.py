...
class ShortestPath(torch.autograd.Function):
    ...
    def forward(ctx, weights, lambda_val):
        ...
        ctx.suggested_tours = np.array([dijkstra(ctx.weights[i,:,:])[0] for i in range(len(ctx.weights))])
    ...
    def backward(ctx, grad_output):
        ...
        better_paths =  np.array([dijkstra(weights_prime[i,:,:])[0] for i in range(len(weights_prime))])
